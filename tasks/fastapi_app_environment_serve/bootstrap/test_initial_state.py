...
def test_fastapi_package_importable():
    """Verify the `fastapi` pip package is installed and importable."""
    result = subprocess.run(
    ...
    assert result.returncode == 0, (
        f"`import fastapi` failed — the fastapi package does not appear to be installed. "
        f"stderr: {result.stderr.strip()}"
    )
